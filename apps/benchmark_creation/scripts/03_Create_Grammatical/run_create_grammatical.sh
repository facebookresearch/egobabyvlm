...
#SBATCH --job-name=devbench-create-grammatical
#SBATCH --nodes=1
#SBATCH --gpus=4
#SBATCH --mem=128G
#SBATCH --time=24:00:00
#SBATCH --output=MachineDevBench_logs/slurm-%j-create-grammatical.out
#SBATCH --error=MachineDevBench_logs/slurm-%j-create-grammatical.err
# Stage 3: Build grammatical 2-AFC benchmark via 3-LLM-call pipeline.
# Launches a vLLM server, runs the benchmark builder, then shuts down.
#
# Usage:
#   bash run_create_grammatical.sh --vocab-dir data/coco_TIMESTAMP --name COCO \
