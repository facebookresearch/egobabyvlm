...
#SBATCH --job-name=devbench-build-nouns
#SBATCH --nodes=1
#SBATCH --gpus=4
#SBATCH --mem=128G
#SBATCH --time=24:00:00
#SBATCH --output=MachineDevBench_logs/slurm-%j-build-nouns.out
#SBATCH --error=MachineDevBench_logs/slurm-%j-build-nouns.err
# Stage 2: Build lexical noun word lists (WordNet categorization + optional LLM filtering).
#
# Without LLM (no vLLM needed):
#   bash run_build_nouns.sh --vocab-dir data/coco_TIMESTAMP --name COCO
#
# With LLM filtering (launches a vLLM server automatically):
#   bash run_build_nouns.sh --vocab-dir data/coco_TIMESTAMP --name COCO \
...
# Build args: inject --api-base if we launched a server
EXTRA_ARGS=()
if [[ -n "$VLLM_MODEL" ]]; then
    EXTRA_ARGS+=(--api-base "http://localhost:${VLLM_PORT}/v1")
fi

python -m apps.benchmark_creation.pipeline.lexical.build_nouns "${EXTRA_ARGS[@]}" "$@"
