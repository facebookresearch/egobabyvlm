...
def get_args_parser(
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Slurm account (required for SLURM submission)",
    )
    parser.add_argument(
        "--qos",
        type=str,
        default=None,
        help="Slurm quality of service (required for SLURM submission)",
    )
    return parser
...
def submit_jobs(task_class, args, name: str):
    ...
    executor.update_parameters(slurm_setup=[
        f"export TRITON_CACHE_DIR=/tmp/triton_cache_{os.environ['USER']}",
        "unset HTTP_PROXY HTTPS_PROXY http_proxy https_proxy",
    ])

    logger.info(f"Executor parameters: {executor_params}")
    ...
    str_output_dir = output_dir.replace("%j", str(job.job_id))
