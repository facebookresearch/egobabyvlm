...
def _parse_slurm_node_list(s: str) -> list[str]:
    ...
    # Pattern matches: node name (with hyphens) optionally followed by bracket expansion
    p = re.compile(r"([a-zA-Z0-9-]+)(?:\[([^\]]+)\])?")
    for m in p.finditer(s):
        prefix = m.group(1)
        suffixes = m.group(2)
        if suffixes:
            # Handle bracket expansion like [053,093] or [1-5]
            for suffix in suffixes.split(","):
                span = suffix.split("-")
                if len(span) == 1:
                    nodes.append(prefix + span[0])
                else:
                    width = len(span[0])
...
def enable(*, set_cuda_current_device: bool = True, overwrite: bool = False, allow_nccl_timeout: bool = False):
    ...
    # Check if already enabled and handle accordingly
    if _LOCAL_RANK >= 0 or _LOCAL_WORLD_SIZE >= 0:
        if not overwrite:
            raise RuntimeError("Distributed mode has already been enabled")
        # If overwrite=True, just return early since it's already enabled
        return
