...
"""Bundled DINOv2 logging: local-file only. The upstream version streamed logs to
remote storage; the OSS port replaces that with a plain
``logging.FileHandler``."""
    ...
    *,
    ...
    Args:
        output: A file name or a directory to save log files. If None, log
            files will not be saved. If ``output`` ends with ".txt" or ".log",
            it is assumed to be a file name; otherwise, logs are saved to
            ``output/log.txt``.
