...
"""HowTo100M frame subset dataset (local-FS only)."""
...
class HowToSubset(ExtendedVisionDataset):
    """HowTo100M frame dataset reading images from a local directory tree.
    ...
    def get_target(self, index: int) -> int:  # noqa: ARG002 -- HowTo frames have no labels.
        return index
    ...
    def dump_extra(self) -> None:
        """Enumerate frames under ``root`` and persist the train/val entries cache."""
        logger.info("Creating entries for HowTo100M %s split", self.split.value)
