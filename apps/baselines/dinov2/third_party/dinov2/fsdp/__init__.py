...
def get_fsdp_wrapper(model_cfg, modules_to_wrap=set()):
    ...
    if model_cfg.mixed_precision.param_dtype == "fp32":
...
def free_if_fsdp(x):
    """
    Free FSDP parameters if the module is sharded.
    Updated to work with newer PyTorch versions where _handles is not available.
    """
    if is_sharded_fsdp(x):
        # Option 1: Use the newer FSDP API
        try:
            # For newer PyTorch versions, use the public API
            x._free_full_params([])
        except AttributeError:
            # Option 2: Try alternative approach for parameter freeing
            try:
                # Some versions may have different internal structure
                if hasattr(x, "_handle") and x._handle is not None:
                    # Single handle case
                    _reshard(x, [x._handle], [True])
                elif hasattr(x, "_handles") and x._handles:
                    # Multiple handles case (original code)
                    handles = x._handles
                    true_list = [True for h in handles]
                    _reshard(x, handles, true_list)
            except (AttributeError, TypeError):
                # Option 3: Fallback - just skip the resharding
                # This might be less memory efficient but won't crash
                pass
...
class FSDPCheckpointer(Checkpointer):
    ...
    def _load_file(self, filename: str):
        """
        Load a checkpoint file. Override to handle PyTorch 2.6+ weights_only default change.
        """
        if filename.endswith(".pkl"):
            # Detectron2-style checkpoint
